...
[Serializable]
public class IOLoginCardSave
{
    public uint profession;
    public List<uint> cardList = new List<uint>();
}

public static class IOHelper
{
    private const string PRIMITIVE_KEY = "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"; 
    ...
    public static void SetData(string fileName, object pObject)
    {
        ...
        //对字符串进行加密,32位加密密钥
        toSave = RijndaelEncrypt(toSave, PRIMITIVE_KEY);
        StreamWriter streamWriter = File.CreateText(GetFileLocalSave(fileName));
    ...
    {
        string localFile = GetFileLocalSave(fileName);
        if (!IsFileExists(localFile)) return default(T);

        StreamReader streamReader = File.OpenText(localFile);
        string data = streamReader.ReadToEnd();

        //对数据进行解密，32位解密密钥
        data = RijndaelDecrypt(data, PRIMITIVE_KEY);
        streamReader.Close();

        object obj = DeserializeObject(data, typeof(T));
